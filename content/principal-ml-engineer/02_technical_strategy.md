...
- Structure a strategy using the Rumelt kernel — falsifiable diagnosis, guiding policies each with an explicit forbids clause, coherent sequenced actions — rather than producing a roadmap or wish list
- Run an ML org diagnosis: model inventory, pipeline and stack count, loaded people-cost by activity smeared across teams, and constraint identification by time-lining real launches rather than asking team leads what they want
- Write a BLUF-first strategy document where the bottom line survives being read aloud in three sentences to someone with no context, and where the asks section names decision-maker, deadline, and the default outcome if no decision is made
- Socialize a strategy through pre-wiring — walking the document through 1:1s with every "no" that could kill it before the formal review, converting objectors into minor co-authors by visibly incorporating what survives their attacks
- Write revisit triggers tied to the strategy's load-bearing assumptions at the time of writing, so updates happen on explicit signal rather than on inertia or the annual planning calendar
